...
export async function fetchAdAccountMinimumBudget(
  ...
  try {
    const account = await get(`/${actId}`, accessToken, { fields: 'currency,min_daily_budget' });
...
export function validateBoostInput(input: BoostCreateInput, currency: string, minDailyMinor: number | null): string[] {
  ...
  if (Number.isNaN(start.getTime()) || Number.isNaN(end.getTime())) {
    ...
    if (end.getTime() - start.getTime() < META_DAILY_BUDGET_MIN_MS) {
...
export async function createBoost(input: BoostCreateInput, transport: BoostMetaTransport = defaultBoostTransport()) {
  ...
  input = { ...input, websiteUrl: normalizeBoostWebsiteUrl(input.objective, input.websiteUrl) };
  ...
  if (objective.key === 'profile_visits') {
    ...
    if (!username) {
      return { success: false, error: { code: 'VALIDATION_ERROR', message: 'Instagram username is required for Profile Visits.' } };
    }
